...
canny_count, stream_count, mask_count = 0, 0, 0
...
class Network(customtkinter.CTk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.entry = customtkinter.CTkEntry(master = self.frame_right, width = 120, placeholder_text = "Enter path to export code and dump screenshots")
    ...
    def stream(self):    
        ...
        if ret:
            ...
            contours = Network.getContours(mask)
            for contour in contours:         
                approx = cv.approxPolyDP(contour, 0.01 * cv.arcLength(contour, True), True)
                x, y, w, h = cv.boundingRect(contour)
                if self.single_target.get() == 1:
    ...
    def take_screenshot(self):
        ret, frame = self.cam.get_feed()
        if ret:
            print("saved", self.entry.get())
            cv.imwrite(os.path.join(self.entry.get(), ("stream-" + time.strftime("%d-%m-%Y-%H-%M-%S") + ".png")), frame)
